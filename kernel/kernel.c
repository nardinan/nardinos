...
void main() {
  ...
  port_byte_write(0x3d4, 14); /* Requesting byte 14: high byte of cursor pos */
  /* Data is returned in VGA data register (0x3d5) */
  int position = port_byte_read(0x3d5);
  position = position << 8; /* high byte */

  port_byte_write(0x3d4, 15); /* requesting low byte */
  position += port_byte_read(0x3d5);
