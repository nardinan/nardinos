...
void *memory_alloc(size_t bytes) {
  ...
  if (memory.elements < MEMORY_MAXIMUM_FRAGMENTS) {
    ...
    if (memory.fragments[index].flags & e_memory_flag_memory_initialized) {
      ...
      /* if we got some residual memory, we can create a new fragment to keep 
       * it in order to let somebody else use it. This is pretty convenient, 
       * especially if the block we've selected is pretty big and we need just
      ...
      if (residual_memory > 0) {
        ...
        memory.fragments[index].end = (memory.fragments[index].start + required_size);
        memory.fragments[index + 1].start = memory.fragments[index].end;
        memory.fragments[index + 1].flags = e_memory_flag_memory_initialized;
        ++(memory.elements);
      }
    } else {
      if (index > 0)
        memory.fragments[index].start = memory.fragments[index - 1].end;
      else
...
void memory_free(void *pointer) {
  if (pointer) {
    ...
    memory_copy((unsigned char *)&bytes, casted_pointer, sizeof(size_t));
    for (size_t index = 0;
        (index < memory.elements) &&
        (memory.fragments[index].flags & e_memory_flag_memory_initialized);
        ++index) {
        ...
        if (memory.fragments[index].flags & e_memory_flag_memory_taken) {
          ...
          if ((head_residual_memory) && 
            ...
            required_slots = 2;
              ...
              (tail_residual_memory))
            required_slots = 1;
          if (memory.elements < (MEMORY_MAXIMUM_FRAGMENTS - (required_slots - 1))) {
            ...
            if (head_residual_memory) {
              memory_move((unsigned char *)&(memory.fragments[index + 1]), 
                  (unsigned char *)&(memory.fragments[index]),
                  (sizeof(s_memory_fragment) * (memory.elements - index)));
              memory.fragments[index].end = casted_pointer;
            ...
            if (tail_residual_memory) {
              if (index < (memory.elements - 1))
                memory_move((unsigned char *)&(memory.fragments[index + 2]),
                    ...
                    (sizeof(s_memory_fragment) * (memory.elements - index - 1)));
              memory.fragments[index + 1].end = memory.fragments[index].end;
              memory.fragments[index].end = (casted_pointer + bytes + sizeof(size_t));
              memory.fragments[index + 1].start = memory.fragments[index].end;
              /* we make clear that the new block created (or copyed) contains memory 
               * that has been initialized. This is important in case we're creating a
               * new block in a non-yet-used area
               */
              memory.fragments[index + 1].flags |= e_memory_flag_memory_initialized;
              ++(memory.elements);
            }
